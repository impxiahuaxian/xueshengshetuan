...
{
	// This command is called only when an img element is selected (IMG).
	// Get img URL (src).
...
FCKCommands.RegisterCommand( 'Openimg', oMyCMCommand ) ;
...
{
	// Let's show our custom option only for img.
	if ( tagName == 'IMG' )
	{
		contextMenu.AddSeparator() ;
		contextMenu.AddItem( 'Openimg', 'Open img in a new window (Custom)' ) ;
	}
}
