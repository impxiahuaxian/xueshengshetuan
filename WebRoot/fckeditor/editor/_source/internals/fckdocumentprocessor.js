...
var FCKDocumentProcessor_CreateFakeimg = function( fakeClass, realElement )
{
	...
	FCKAnchorsProcessor.ProcessDocument = function( document )
	{
		...
		while ( i >= 0 && ( oLink = aLinks[i--] ) )
		{
			...
			if ( oLink.name.length > 0 )
			{
				...
				{
					var oImg = FCKDocumentProcessor_CreateFakeimg( 'FCK__Anchor', oLink.cloneNode(true) ) ;
...
FCKPageBreaksProcessor.ProcessDocument = function( document )
{
	...
	while ( i >= 0 && ( eDIV = aDIVs[i--] ) )
	{
		...
		{
			var oFakeimg = FCKDocumentProcessor_CreateFakeimg( 'FCK__PageBreak', eDIV.cloneNode(true) ) ;

			eDIV.parentNode.insertBefore( oFakeimg, eDIV ) ;
...
/*
	...
	while ( i >= 0 && ( oCenter = aCenters[i--] ) )
	{
		...
		{
			var oFakeimg = FCKDocumentProcessor_CreateFakeimg( 'FCK__PageBreak', oCenter.cloneNode(true) ) ;

			oCenter.parentNode.insertBefore( oFakeimg, oCenter ) ;
...
FCKEmbedAndObjectProcessor = (function()
{
	...
	var processElement = function( el )
	{
		...
		var replaceElement ;
		var fakeImg = replaceElement = FCKDocumentProcessor_CreateFakeimg( 'FCK__UnknownObject', clone ) ;
...
FCKDocumentProcessor.AppendNew().ProcessDocument = function( document )
{
	...
	while ( i >= 0 && ( oInput = aInputs[i--] ) )
	{
		...
		{
			var oImg = FCKDocumentProcessor_CreateFakeimg( 'FCK__InputHidden', oInput.cloneNode(true) ) ;
