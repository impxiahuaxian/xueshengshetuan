...
//	e.style.pageBreakAfter = 'always' ;
	...
	var oFakeimg = FCKDocumentProcessor_CreateFakeimg( 'FCK__PageBreak', e ) ;
	var oRange = new FCKDomRange( FCK.EditorWindow ) ;
	oRange.MoveToSelection() ;
	var oSplitInfo = oRange.SplitBlock() ;
	oRange.InsertNode( oFakeimg ) ;
...
FCKAnchorDeleteCommand.prototype =
{
	Execute : function()
	{
		...
		{
			var oFakeimg = FCK.Selection.GetSelectedElement() ;
			if ( oFakeimg )
			{
				if ( oFakeimg.tagName == 'IMG' && oFakeimg.getAttribute('_fckanchor') )
					oAnchor = FCK.GetRealElement( oFakeimg ) ;
				else
					oFakeimg = null ;
			}
			...
			// If we got a fake img, then just remove it and we're done
			if ( oFakeimg )
			{
				oFakeimg.parentNode.removeChild( oFakeimg ) ;
				return ;
			}
