...
<!---
Check file content.
Currently this function validates only img files.
Returns false if file is invalid.
detectionLevel:
	0 = none
	1 = check img size for img,
	2 = use DetectHtml for img
---->
<cffunction name="IsimgValid" returntype="boolean" output="true">
	...
	<cftry>
		<cfif REQUEST.CFVersion gte 8>
			<cfset objimg = imgRead(ARGUMENTS.filePath) >
			<cfset imgInfo = imgInfo(objimg)>
			<!--- <cfimg action="info" source="#ARGUMENTS.filePath#" structName="imgInfo" /> --->
		<cfelse>
			<cfset imgCFC = createObject("component", "img")>
			<cfset imgInfo = imgCFC.getimgInfo("", ARGUMENTS.filePath)>
		</cfif>

		<cfif imgInfo.height lte 0 or imgInfo.width lte 0>
			<cfreturn false>
		</cfif>
	<cfcatch type="any">
		<cfreturn false>
