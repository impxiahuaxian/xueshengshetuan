var FCKDragTableHandler =
{
	...
	"_ResizeBarMouseDownListener" : function( evt )
	{
		...
		if ( FCKBrowserInfo.IsIE )
			FCKDragTableHandler._ResizeBar.filters.item("DXimgTransform.Microsoft.Alpha").opacity = 50 ;
		else
	...
	"_ShowResizeBar" : function( w, table, mouse )
	{
		if ( this._ResizeBar == null )
		{
			...
			if ( FCKBrowserInfo.IsIE )
				paddingStyles.filter = "progid:DXimgTransform.Microsoft.Alpha(opacity=10,enabled=true)" ;
			else
			...
			// So we need to create a spacer img to fill the block up.
			...
			// Disable drag and drop, and selection for the filler img.
		...
		if ( FCKBrowserInfo.IsIE )
			paddingBar.filters.item("DXimgTransform.Microsoft.Alpha").opacity = 10 ;
		else
